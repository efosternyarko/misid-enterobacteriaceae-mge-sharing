...
::: {.callout-warning}
**Flagged for review, not yet independently verified**: this script's isolate-name matching (`if isolate_base in d`) checks whether the ICEfinder2 result directory name is contained *within* a shorter `01_mob_recon/` directory name. Given the ICEfinder2 result directory retains a longer name (including the `_chromosome_merged` suffix from the merge step) than the `01_mob_recon/` directory it needs to match against, this containment check runs in the direction that would not normally succeed as a substring match. It is reproduced here unchanged from the working version supplied, since altering matching logic without confirming actual ICEfinder2 output directory naming behaviour risked introducing a different, unverified bug — but this is worth Ronnie/Ebenn checking directly against real output before relying on it. See `NOTES_FOR_REVIEW.md`.
:::
