...
author: "Cheuk Kiu Liu (Ronnie) · Supervisor: Dr E. Foster-Nyarko"
date: "26 July 2026"
format:
  html:
    toc: true
...
::: {.callout-note}
**Isolate-name matching fixed and confirmed 2026-07-26.** The original supplied version had `if isolate_base in d:`, checking whether the *longer* ICEfinder2 result directory name (which retains the `_chromosome_merged` suffix from the merge step, e.g. `EC_E18_Ecoli_chromosome_merged`) is contained within the *shorter* `01_mob_recon/` directory name (e.g. `EC_E18_Ecoli`) — a containment check that can never succeed, since a longer string cannot be a substring of a shorter one. Confirmed directly against real output on CLIMB (`ls 01_mob_recon/` vs `ls ICEfinder2_linux/result/`): the mob_recon directories are exactly the ICEfinder2 directory names with `_chromosome_merged` stripped off the end. Corrected to `if d in isolate_base:` (does the short mob_recon name appear within the long ICEfinder2 name — the direction that actually matches).
:::
