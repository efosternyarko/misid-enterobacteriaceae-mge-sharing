...
5. **`extract_ice_sequences.py`'s isolate-name matching (2026-07-26, confirmed against real CLIMB output)** — the original `if isolate_base in d:` checked whether the *longer* ICEfinder2 result directory name (e.g. `EC_E18_Ecoli_chromosome_merged`) was contained within the *shorter* `01_mob_recon/` directory name (e.g. `EC_E18_Ecoli`) — impossible, since a longer string can't be a substring of a shorter one. Confirmed directly via `ls` on both directories on CLIMB: mob_recon names are exactly the ICEfinder2 names with `_chromosome_merged` stripped. Fixed to `if d in isolate_base:`.
